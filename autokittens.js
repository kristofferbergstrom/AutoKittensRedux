//Based on https://github.com/recalibr8/AutoKittens/
//Original source: http://birdiesoft.dk/autokittens.js
...
function buildUI() {
  ...
  uiContainer.style.height = '500px';
  ...
  calcContainer.style.backgroundColor = '#ececec';
  calcContainer.style.position = 'absolute';
  calcContainer.style.top = '0%';
   calcContainer.style.right = '0%';
  calcContainer.style.padding = '40px';
  calcContainer.style.overflowY = 'scroll';
  calcContainer.style.lineHeight = '2em';
  calcContainer.style.height = '500px';
  calcContainer.style.width = '600px';
  calcContainer.style.marginLeft = '300px';
...
function rebuildOptionsUI() {
  ...
  autoTradeRaces = [["No one", ""]];
  gamePage.diplomacy.races.forEach(function (r) {
    if (r.unlocked) {
      autoTradeRaces.push([r.title || r.name, r.name]);
    }
  });
  addCheckbox(uiContainer, 'autoOptions.tradeOptions', 'tradeLeviathans', 'Always trade with the Leviathans first when available');
  addOptionMenu(uiContainer, 'autoOptions.tradeOptions', 'tradePartner', 'Trade with', autoTradeRaces, 'by default');
  addCheckbox(uiContainer, 'autoOptions.tradeOptions', 'suppressTradeLog', 'Hide log messages when auto-trading');
  autoTradeRaces[0][0] = "Default selection";
  ...
  addIndent(uiContainer);addOptionMenu(uiContainer, 'autoOptions.tradeOptions', 'tradePartnerSpring', 'Trade with', autoTradeRaces, ' in spring');
  addCheckbox(uiContainer, 'autoOptions.tradeOptions', 'tradeSummer', 'Allow trading in summer');
  addIndent(uiContainer);addOptionMenu(uiContainer, 'autoOptions.tradeOptions', 'tradePartnerSummer', 'Trade with', autoTradeRaces, ' in summer');
  addCheckbox(uiContainer, 'autoOptions.tradeOptions', 'tradeAutumn', 'Allow trading in autumn');
  addIndent(uiContainer);addOptionMenu(uiContainer, 'autoOptions.tradeOptions', 'tradePartnerAutumn', 'Trade with', autoTradeRaces, ' in autumn');
  addCheckbox(uiContainer, 'autoOptions.tradeOptions', 'tradeWinter', 'Allow trading in winter');
  addIndent(uiContainer);addOptionMenu(uiContainer, 'autoOptions.tradeOptions', 'tradePartnerWinter', 'Trade with', autoTradeRaces, ' in winter');
  ...
  addIndent(uiContainer);addInputField(uiContainer, 'autoOptions.craftOptions', 'bufferSteel', 'Save up at least', 'steel before crafting');
  ...
  addIndent(uiContainer);addInputField(uiContainer, 'autoOptions.craftOptions', 'thoriumAmunt', 'Craft', 'thorium at a time');
  ...
  addIndent(uiContainer);addInputField(uiContainer, 'autoOptions.craftOptions', 'bufferParchments', 'Save up at least', 'parchment(s) before crafting');
  ...
  addIndent(uiContainer);addInputField(uiContainer, 'autoOptions.craftOptions', 'bufferManuscripts', 'Save up at least', 'manuscript(s) before crafting');
  ...
  addIndent(uiContainer);addInputField(uiContainer, 'autoOptions.craftOptions', 'bufferCompendiums', 'Save up at least', 'compendium(s) before crafting');
...
starClick = function () {
  if (autoOptions.autoStar) {
	 if(gamePage.calendar.observeBtn != null) {gamePage.calendar.observeBtn.click();}
  }
}
...
calculateCraftAmounts = function() {
  var resources = ["wood", "beam", "slab", "steel", "plate", "alloy", "parchment", "manuscript", "blueprint", "compedium", "kerosene", "thorium", "eludium"]
  for (var i = 0; i < resources.length; i++) {
...
autoCraft = function () {
  ...
  var resources = [
    ...
    ["titanium", "alloy",      "craftAlloy",      gamePage.science.get('construction').researched && (gamePage.resPool.get('steel').value >= autoOptions.craftOptions.bufferSteel + 75 * autoOptions.craftOptions.alloyAmount)],
    ["oil",      "kerosene",   "craftKerosene",   gamePage.science.get('construction').researched],
	["uranium",  "thorium",    "craftThorium",   	gamePage.science.get('construction').researched],
    ...
    ["culture",  "manuscript", "craftManuscript", gamePage.science.get('construction').researched && (!autoOptions.craftOptions.festivalBuffer || gamePage.resPool.get('parchment').value > 2500 + 25 * autoOptions.craftOptions.manuscriptAmount) && (gamePage.resPool.get('parchment').value >= autoOptions.craftOptions.bufferParchments + 25 * autoOptions.craftOptions.manuscriptAmount)],
    ["science",  "blueprint",  "craftBlueprint",  gamePage.science.get('construction').researched && autoOptions.craftOptions.blueprintPriority && (gamePage.resPool.get('compedium').value >= autoOptions.craftOptions.bufferCompendiums + 25 * autoOptions.craftOptions.blueprintAmount)],
    ["science",  "compedium",  "craftCompendium", gamePage.science.get('construction').researched && (gamePage.resPool.get('manuscript').value >= autoOptions.craftOptions.bufferManuscripts + 50 * autoOptions.craftOptions.compediumAmount)],
    ["science",  "blueprint",  "craftBlueprint",  gamePage.science.get('construction').researched && !autoOptions.craftOptions.blueprintPriority && (gamePage.resPool.get('compedium').value >= autoOptions.craftOptions.bufferCompendiums  + 25 * autoOptions.craftOptions.blueprintAmount)]
  ];
  for (var i = 0; i < resources.length; i++) {
...
autoTrade = function () {
  ...
  if (autoOptions.tradeOptions.tradeLeviathans) {
	var race = gamePage.diplomacy.get("leviathans");
    tradeWithRace(race, 1);
  }
  var race = undefined;
  ...
  if(race !== undefined  && autoOptions.tradeOptions['trade' + season])
  {
	var gold = gamePage.resPool.get('gold');
	if (gold.value / gold.maxValue >= autoOptions.tradeOptions.tradeLimit)
	{
		tradeOptimal(race);
		//tradeWithRace(race, autoOptions.tradeOptions.tradeCount);
	}
  }
  
}

function tradeOptimal(autoTradeRace)
{
	var raceTrade = autoTradeRace.buys[0];
	var tradeItem = gamePage.resPool.get(raceTrade.name);
	
	var manpowerPerTick = gamePage.resPool.get("manpower").perTickCached;
	var goldPerTick = gamePage.resPool.get("gold").perTickCached;
	
	var goldTrades = Math.floor(goldPerTick / 50);
	var manpowerTrades = Math.floor(manpowerPerTick / 50);
	var raceTrades = Math.floor(tradeItem.value / raceTrade.val);
	
	var optimalAmt = Math.min(goldTrades, manpowerTrades, raceTrades).toFixed(0);
	if(optimalAmt < 1) optimalAmt = 1;
	
	tradeWithRace(autoTradeRace, optimalAmt);
}

tradeWithRace = function (autoTradeRace, amt) {
  if (!autoTradeRace.unlocked) {
    return false;
  }
  if (gamePage.resPool.get(autoTradeRace.buys[0].name).value < autoTradeRace.buys[0].val * amt || gamePage.resPool.get("manpower").value < 50 * amt ||  gamePage.resPool.get("gold") < 15 * amt) {
    return false;
  }
  var msgFunc = gamePage.msg;
  ...
  gamePage.diplomacy.tradeMultiple(autoTradeRace, amt);
...
autoFestival = function () {
  if (gamePage.calendar.festivalDays || !autoOptions.autoFestival || !gamePage.science.get('drama').researched)
    return;
  if (!(game.resPool.get("manpower").value >= 1500 &&
	  game.resPool.get("culture").value >= 5000 &&
	  game.resPool.get("parchment").value >= 2500))
	{ return;}
...
function calculateBaseUps(extras) {
  ...
  var paragonRatio = gamePage.resPool.get("paragon").value * 0.01;
	paragonRatio = 1 + gamePage.getHyperbolicEffect(paragonRatio, 2);
...
function buildingCalculator() {
  ...
  if (gamePage.spaceTab.visible) {
    ...
    var planets = gamePage.space.planets.slice(0);
    planets.sort(function(a, b){return a.label.localeCompare(b.label)});
    
	for (var i = 0; i < planets.length; i++) {
      if (planets[i].unlocked){//&& space[i].upgradable) {
        var buildings = planets[i].buildings;
		buildings.sort(function(a, b){return a.label.localeCompare(b.label)});
		
		for (var j = 0; j < buildings.length; j++) {
		  if (buildings[j].unlocked){//&& space[i].upgradable) {
		     result += '<option value="space_'+planets[i].label+'">'+planets[i].label+ ": " + buildings[j].label +'</option>';
		  }
        }
	  }
    }
  ...
  for (var i = 0; i < prices.length; i++) {
    ...
    if (bldName[0] == 'space' && (prices[i].name == "oil" || prices[i].name == "rocket")) {
      ...
      if (prices[i].name == "oil")
				reductionRatio = gamePage.getHyperbolicEffect(gamePage.getEffect("oilReductionRatio"), 0.75);
...
function mintCalculator() {
  var hunterRatio = gamePage.getEffect("hunterRatio");
  ...
  var ipsWithMint = expectedIvory/huntTimeWithMint;
	...
	var cpratio = (catpower.maxValue * gamePage.bld.get("mint").effects["manpowerPerTickCon"]) / 100;
